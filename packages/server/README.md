...
The client side (`@mbsks/openrtc-core` + pluggable signaling adapters) is the media
...
```
App (vibe coder)
   │  @mbsks/openrtc-core (client engine: RTCPeerConnection mesh, adapters)
   ▼
@mbsks/openrtc-server  ── rooms · roster · signal log · relay · recordings
   │  Store (function-based, ~10 methods)          RecordingStorage (bytes)
   ├── InMemoryStore (default entry)               ├── DiskRecordingStorage
   ├── SqliteStore   (@mbsks/openrtc-server/stores/sqlite)   └── S3RecordingStorage
   ├── PostgresStore (@mbsks/openrtc-server/stores/postgres)   (fetch + SigV4, no AWS SDK)
   └── MysqlStore    (@mbsks/openrtc-server/stores/mysql)
   │  Hosting
   ├── node:http standalone  (createNodeServer)
   ├── Express router        (@mbsks/openrtc-server/express)
   ├── Fastify plugin        (@mbsks/openrtc-server/fastify)
...
```sh
npm install @mbsks/openrtc-server
```
...
and its only runtime dependencies are `@mbsks/openrtc-protocol` and the pure-JS
...
(same `types` / `development` / `default` convention as `@mbsks/openrtc-transport`):

| Subpath                           | Exports                | Also install           |
| --------------------------------- | ---------------------- | ---------------------- |
| `@mbsks/openrtc-server`                 | everything below plus… | —                      |
| `@mbsks/openrtc-server/stores/sqlite`   | `SqliteStore`          | `npm i better-sqlite3` |
| `@mbsks/openrtc-server/stores/postgres` | `PostgresStore`        | `npm i pg`             |
| `@mbsks/openrtc-server/stores/mysql`    | `MysqlStore`           | `npm i mysql2`         |
| `@mbsks/openrtc-server/express`         | `createExpressRouter`  | `npm i express`        |
| `@mbsks/openrtc-server/fastify`         | `createFastifyPlugin`  | `npm i fastify`        |

Notes:

- SQL stores load their driver lazily and only inside the store module:
  importing `@mbsks/openrtc-server` never resolves `pg` / `mysql2` /
...
import { createServices, InMemoryStore, attachWebSocketRelay } from '@mbsks/openrtc-server';
...
```ts
import { issueToken } from '@mbsks/openrtc-server';
...
All four pass the **shared store test suite** (`@mbsks/openrtc-server/shared-tests`,
...
| ---------------------------------- | -------- | ------------ | ------ | -------------------------------------------- |
| `@mbsks/openrtc-protocol`                | `0.1.0`  | in-workspace | —      | wire envelope types (single source of truth) |
