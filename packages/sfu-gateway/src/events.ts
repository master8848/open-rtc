/**
 * Typed event emitter — in-workspace mirror of `@mbsks/openrtc-core`'s `TypedEmitter`
 * (zero runtime dependencies; same shape as Node's EventEmitter but typed and
 * framework-agnostic). Kept in-package so `@mbsks/openrtc-sfu-gateway` stays a
 * leaf package: `Room` will depend on this package, not the other way around.
 *
 * ```ts
 * interface MyEvents { 'data': [number]; 'error': [Error]; }
