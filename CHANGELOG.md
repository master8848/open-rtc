...
- **@mbsks/openrtc-react (new package).** `useRoomState`, `useParticipants`,
...
- examples/react: stable `selfId` fixes a StrictMode ghost roster entry; the
  example now consumes the `@mbsks/openrtc-react` hooks.
...
- `@mbsks/openrtc-transport`: transport twin + helpers (chunker, reorder buffer,
...
- Examples: vanilla two-tab BroadcastChannel call, React + Supabase room,
  Express + `@mbsks/openrtc-server` host.
