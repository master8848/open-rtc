...
| Postgres `NOTIFY` | `seq-required` | 7 KB (chunker) | `LISTEN` must be a **dedicated `pg.Client`**, not the pool (`docs/architecture.md:127`) | yes — use `PostgresNotifyRelay` or the WS bridge |
...
- **WS relay:** `Relay` (`packages/server/src/services.ts`) is pluggable. Default
  is `RoomHub` (local). For N instances behind a LB, inject `RedisRelay` (ioredis
  `pub/sub` channel `vidcall:room:{roomId}`) or `PostgresNotifyRelay` (dedicated
  `pg.Client` `LISTEN vidcall_room` + 7 KB chunker). Wire via
  `attachWebSocketRelay(server, services, { relay })` — HTTP `handleSignal` already
  calls `relay.broadcast`.
- **Store:** `PostgresStore` uses a dedicated `listenerClient` for `LISTEN`; `RedisStore`
  (`packages/server/src/stores/RedisStore.ts`, optional, peer `ioredis`/`redis`) uses
  `ZSET` + TTL for `listSignals`. Existing stores keep working; large deploys pick Redis.
