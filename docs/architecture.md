...
optional: SfuGateway adapter (generic SFU protocol; reference impl; mediasoup/LiveKit integration guides)
          ^ landed: MediaTransport seam (mesh vs SFU via single PC), TopologyController auto mesh→SFU at threshold 4, MediaProcessor chain + active-speaker; Room wires SfuGateway via SfuMediaTransport (sfu `SfuPayload` envelopes → SfuSession, offer/answer/ice → handleOffer/handleAnswer/addIceCandidate)
```
...
| D2  | **Mesh = core; SFU = optional `SfuGateway` interface** (not bundled). **Status:** landed — `MediaTransport` seam + `MeshMediaTransport`/`SfuMediaTransport`, `TopologyController` auto mesh→SFU (threshold 4, manual downgrade via `room.setTopology('mesh')`), `MediaProcessor` chain (e2ee last, Safari fallback), active-speaker; `packages/sfu-gateway` interface + `SfuRouter` + mediasoup adapter + tests green (see §4.1); `Room` wiring done (media `sfu` envelope → `SfuMediaTransport.handleSfuEnvelope` → `SfuSession`, `publish({simulcast,svc,codecPreferences})` + `setPreferredLayers`/`requestKeyframe`/`setTile`) | mesh hard limit `4–6 video` is WebRTC peer-to-peer physics (`N−1` uplinks/encodes/decodes, `iOS ≤4 <video>`) not a `vidcall` cap — `Room` `packages/core/src/media/media-transport.ts:47` delegates; solution within lib is `TopologyController` `packages/core/src/media/topology.ts:39` `autoThreshold 4` `maybeMigrate` `mesh→sfu` 1 uplink; fine for 2-4 ("simple Zoom clone"); SFU add-on via reference impl + mediasoup/LiveKit guides |
...
- **Room wiring landed (03-media-topology)**: `Room` selects a `MediaTransport` — `MeshMediaTransport` (N `PeerConnectionManager`s, extracted from Room) or `SfuMediaTransport` (single PC to SFU via `SfuSession` — `publishTrack`/`subscribe`/`setPreferredLayers`/`requestKeyframe` + `handleOffer`/`handleAnswer`/`addIceCandidate`). `Room` delegates `publish`/`unpublish`/`subscribe`/`restartIce`/`getPeerConnections`/`getSenders` to the transport. `sfu` envelopes are routed via `SfuMediaTransport.handleSfuEnvelope` (wire `SfuPayload` → `SfuSession`); `offer`/`answer`/`ice` to/from SFU via `handleEnvelope`. `TopologyController` auto mesh→SFU when participant count exceeds threshold (default 4); manual `room.setTopology('mesh'|'sfu'|'auto')`. `MediaProcessor` chain (`denoise`→`background`→`e2ee`) with `room.useProcessor()` (E2EE last, Safari fallback warning). Simulcast/SVC/codecPreferences via `Room.publish({simulcast,svc,codecPreferences})` + `setPreferredLayers`/`requestKeyframe`/`setTile`. Active-speaker foundation polls `inbound-rtp.audioLevel`.
