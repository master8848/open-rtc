...
| quality adaptation engine | ✓ (`@mbsks/openrtc-quality`) | ✗ | ✗ | ✗ |
| ICE restart | ✓ | ✗ | ✗ | ✓ |
...
11. **Adaptive quality**: mobile bindings can already emit/consume `quality-warning`; porting `@mbsks/openrtc-quality`'s pure tier ladder (stats → action) would give mobile the same adaptation; low urgency for v1 mesh.
