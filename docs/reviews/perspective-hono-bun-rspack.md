...
## A. Hono perspective — is `@mbsks/openrtc-server` portable, and how should signaling be shaped for mounting anywhere?
...
dependencies, not architecture: installing `@mbsks/openrtc-server` drags
...
(`@mbsks/openrtc-server/store-pg`, `…/store-mysql`, `…/store-sqlite`) with the
...
- For embedding inside existing Node/Bun apps (the primary distribution),
  ...
  the `npm install @mbsks/openrtc-server better-sqlite3` step from those guides
...
usage in `@mbsks/openrtc-server` at compile time.
...
primary API of `@mbsks/openrtc-server`, and derive every host (node:http, Express,
...
- **Weight**: it demotes `node:http` and `ws` from required runtime facts to
  ...
  runtime deps, matching `@mbsks/openrtc-core`'s discipline on the other side of
  the wire.
