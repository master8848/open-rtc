...
- **Zero-dep core**: `@mbsks/openrtc-core` has no runtime dependencies (platform
...
- **Server component** (`@mbsks/openrtc-server`) is function-first (`Store` as first
...
| WS relay (server) | hand-rolled | `ws`/`socket.io` | Hand-rolled keeps deps zero in `@mbsks/openrtc-server` core; `ws` already present as dev dep for tests. OK. |
...
- **P0 — Quality engine not wired into Room.** `@mbsks/openrtc-quality` ships a
...
1. **P0** Wire `@mbsks/openrtc-quality` into Room (stats → policy → setParameters /
