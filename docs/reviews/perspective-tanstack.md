...
The **engine layer is genuinely TanStack-grade**: `@mbsks/openrtc-core` and `@mbsks/openrtc-quality`
...
snapshot/state layer and therefore no real adapter story — no `@mbsks/openrtc-react`
...
| Dependency discipline | A− | Zero-dep core/quality/transport is real; `@mbsks/openrtc-server` drags native + DB deps |
...
```
@mbsks/openrtc-core        ← engine + a small subscribable state layer   (exists, minus state layer)
@mbsks/openrtc-react       ← useSyncExternalStore bindings, ~200 LOC       (missing)
```
...
(`@mbsks/openrtc-protocol`, or a `@mbsks/openrtc-transport-contract`) and have core import it —
...
- `@mbsks/openrtc-react`: `useRoom(config)` = instance management; `useRoomState(selector)`
...
|---|---|---|---|
| `@mbsks/openrtc-protocol` | none | none | ✅ perfect leaf |
| `@mbsks/openrtc-quality` | `@mbsks/openrtc-protocol` | none | ✅ pure policy engine, no WebRTC imports (`packages/quality/src/index.ts` docblock) |
| `@mbsks/openrtc-core` | `protocol`, `quality` | none | ✅ zero-dep claim holds (grep over `src` confirms) |
| `@mbsks/openrtc-test-utils` | none | none | ✅ |
| `@mbsks/openrtc-transport` | `protocol` | none (vitest is dev-only) | ✅ |
...
| `@mbsks/openrtc-server` | `protocol`, **better-sqlite3, mysql2, pg, ws** | ⚠️ see below |
| `@mbsks/openrtc-sfu-gateway` | `protocol` | none at runtime (`import type` only, `sfu-gateway/src/mediasoup-adapter.ts:30`) | ✅ clever, ⚠️ types leak (below) |
...
1. **`@mbsks/openrtc-server` hard-depends on three database drivers including a *native*
   ...
   `import { createServer } from '@mbsks/openrtc-server'` pulls every driver, native build
   ...
   and each store should live behind a subpath export (`@mbsks/openrtc-server/store-postgres`).
...
- Lint/prettier are configured and enforced (flat ESLint, `eslint.config.mjs`; root
  ...
  rules (forbid `@mbsks/openrtc-core` importing adapters, forbid deep `../../pkg/src`
...
- **Release/versioning: effectively absent.** Root is `private: true`; all 14 TS
  ...
  (`"@mbsks/openrtc-protocol": "0.1.0"`), which means the first real release requires
...
| 2 | **Ship `@mbsks/openrtc-react`** on `useSyncExternalStore`: `useRoom()`, `useParticipants()`, `usePublication()`, `useQuality()`; keyed instance cache, AbortSignal-aware join, StrictMode-tested | Turns a good engine into a usable library; kills the hand-rolled `examples/react` wiring | new `packages/react`; rewrite `examples/react/src/App.tsx` | M (after #1) |
...
| 4 | **Fix `@mbsks/openrtc-server` weight**: move `pg`/`mysql2`/`better-sqlite3`/`ws` to optional peers; subpath exports per store (`/store-postgres`, …); same treatment for sfu-gateway `/mediasoup` | Removes a native-module compile from every install; aligns with "fits anywhere" positioning | `packages/server/package.json`, `src/stores/index.ts`, `src/index.ts`, `packages/sfu-gateway/src/index.ts` | S |
...
- Policy purity in `@mbsks/openrtc-quality` (`adaptive-quality-controller.ts:1-12`: consumes
