...
`@mbsks/openrtc-server` ships rooms, a REST API, and a WebSocket relay. The default
store keeps everything in memory — perfect for a first run:

```ts
import express from 'express';
import { InMemoryStore, attachWebSocketRelay, createServices } from '@mbsks/openrtc-server';
...
real unmount. It comes from `@mbsks/openrtc-react` alongside two snapshot hooks:
...
5. **Nothing is on npm yet.** Install commands like `npm i @mbsks/openrtc-core`
