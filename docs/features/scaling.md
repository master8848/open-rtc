...
| vidcall fit           | Reference adapter ships in `@mbsks/sfu-gateway`; you own signaling (matches vidcall's model) | Use `livekit-client` directly or a thin adapter — but LiveKit owns the peer connection, so pluggable backends don't apply | GPLv3 forces license review; plugin protocol needs an adapter | Apache-2.0; COLIBRI adapter needed |
