...
mediasoup adapter in `@mbsks/openrtc-sfu-gateway`) is the migration path when a call
...
| **SFU (selective forwarding unit)** | One uplink to a media server; the server forwards to subscribers | `SfuGateway` + `SfuSession` (`@mbsks/openrtc-sfu-gateway`) | 5+ video participants, webinars, large classes, recording/egress needs       |
