# Hosting @mbsks/openrtc-server in a Fastify app
...
```bash
npm install @mbsks/openrtc-server fastify pg
```

```ts
// server.ts
import Fastify from 'fastify';
import { attachWebSocketRelay, createServices, S3RecordingStorage } from '@mbsks/openrtc-server';
import { createFastifyPlugin } from '@mbsks/openrtc-server/fastify';
